...
MemoLane.TimeLine.prototype.makeBody = function(item, body, data, callback) {
  ...
  if (storyId != "") {
    if( editMode ) {
      ...
      Gnarly.json(['get', [url+".json", {from: item.from, to: item.to}], {}], function(xhr, result) {
        if (xhr.status === 200) {

          var ids = [];
          $.each(memosdata.memos, function(i, memo) {
            if( memo )
              ids.push( memo.id );
          });
...
MemoLane.createSlots = function( centerTime ) {
  ...
  MemoLane.memoDeepLinkId = hash.search(/memo=/)>-1 ? hash.substring(hash.search(/memo=/)).substring(5).split('?')[0] : null;
   ...
   //$("#urls .volume").attr("href");

  if( url != "" ) {
    //TODO:REPLACE volume
  Gnarly.json(['get', url+".date"], function(xhr, data) { //get volume json object
      if (xhr.status === 200) {
        if( data ) {
          var list = new Array();
          var timeSet = new Array();
          var lastTo = 0;
            $.each(data, function(i,fromTime) {
                var toTime = parseInt(fromTime) + 86300; //add 24 hours in seconds to get fromTime
                if( lastTo < showAt && showAt <= toTime ){
                    MemoLane.initialIndex = timeSet.length;
                }
                ...
                list.push(s); //make array containing unix time stamp with time range in 24 hours
            });
          if( MemoLane.initialIndex != 0 ){
...
//TODO:REPLACE members
        var data=[{
            "id":"d6a3fd7863fa1000f1b0c80adc1a9c9d","username":"joestainbrook","first_name":"Joe","last_name":"Stainbrook","image":"http://a2.twimg.com/profile_images/517981154/twitter_normal.JPG",
            "owner":true},{"id":"a1c6f3afb64ba11ddf05a7d94450e487","username":"xmpty","first_name":"Mike","last_name":"Kamowski","image":"https://graph.facebook.com/32811975/picture","owner":false
        }];
      // Gnarly.json(['get', url], function(xhr, data) {
        // if (xhr.status == 200) {
          $.each( data, function(i, member) {
